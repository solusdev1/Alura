...
function Send-ToServer {
    ...
    try {
        ...
        $response = Invoke-RestMethod -Uri $url -Method Post -Headers $headers -Body $jsonBytes -ContentType "application/json; charset=utf-8" -TimeoutSec $timeout
...
function Get-PublicIP {
    try {
        $response = Invoke-RestMethod "https://api.ipify.orgformat=json" -TimeoutSec 5
        return $response.ip
    } catch {
        return $null
    }
