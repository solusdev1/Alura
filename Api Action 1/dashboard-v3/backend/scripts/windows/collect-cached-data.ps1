...
param(
    ...
    [string]$ApiSecret = $env:SAVE_DISPLAY_NAME_SECRET,
...
try {
    if ([string]::IsNullOrWhiteSpace($ApiSecret)) {
        throw "SAVE_DISPLAY_NAME_SECRET nao configurado no ambiente ou parametro -ApiSecret."
    }
    ...
    $headers = @{ "x-display-secret" = $ApiSecret }
    $success = $false
    $lastError = $null
    
    try {
        $response = Invoke-RestMethod -Uri $LocalApiUrl -Method Post -Headers $headers -Body $jsonBytes -ContentType "application/json; charset=utf-8" -TimeoutSec $TimeoutSeconds
        
        if ($response.success) {
            Write-Host "   [OK] SUCESSO no servidor LOCAL!" -ForegroundColor Green
            $success = $true
        }
    } catch {
        Write-Host "   [ERRO] Servidor local falhou: $($_.Exception.Message)" -ForegroundColor Yellow
        ...
        try {
            $response = Invoke-RestMethod -Uri $CloudApiUrl -Method Post -Headers $headers -Body $jsonBytes -ContentType "application/json; charset=utf-8" -TimeoutSec $TimeoutSeconds
            
            if ($response.success) {
                Write-Host "   [OK] SUCESSO no servidor CLOUD!" -ForegroundColor Green
                $success = $true
            }
        } catch {
            Write-Host "   [ERRO] Servidor cloud também falhou: $($_.Exception.Message)" -ForegroundColor Red
